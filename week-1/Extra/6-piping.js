...
function badPractice(startingValue) {
	let addTenAndMultipyTwo = (startingValue + 10) * 2;
	return `£${addTenAndMultipyTwo}`;
}
// Why can this code be seen as bad practice? Comment your answer.
const badCode = badPractice(startingValue);
/* BETTER PRACTICE */
// The bad practice is to use to much global scope rather than local (block) scope
const goodPractice = () => {
	const num = 2;
	let sumAndMultipe = (num + 10) * 2;
	return `£${sumAndMultipe}`;
};

let goodCode = goodPractice();
