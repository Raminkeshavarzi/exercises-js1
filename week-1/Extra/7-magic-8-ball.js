...
function shakeBall(ans) {
	let magicAnswer = Math.floor(Math.random() * 4) + 1;
	console.log('The ball has shaken!');
	const theLevelOfpositivity = [ 'very positive', 'positive', 'negative', 'very negative' ];
	for (let i = magicAnswer; i < theLevelOfpositivity.length; i++) {
		return `${theLevelOfpositivity[i]}`;
	}
...
function checkAnswer(answer) {
	...
	if (answer === 'very positive' || answer === 'positive' || answer === 'negative' || answer === 'Very negative') {
		return answer;
	}

	const theLevelOfpositivity = [ 'very positive', 'positive', 'negative', 'very negative' ];
	for (let i = 0; i <= theLevelOfpositivity.length; i++) {
		return `${theLevelOfpositivity[i]}`;
	}
}
...
function testAll() {
	...
	test(`checkAnswer returns different answers`, new Set(Array.from(answers.values()).map(checkAnswer)).size > 1);
}
